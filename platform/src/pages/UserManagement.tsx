...
export default function UserManagement() {
    ...
    // Tab state: 'users' or 'credits'
    const [activeView, setActiveView] = useState<'users' | 'credits'>('users');
        ...
        <div className="space-y-8 animate-in fade-in duration-700 container mx-auto py-8">
            ...
            <div className="flex gap-2 mb-2">
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 ${activeView === 'users'
                ...
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 ${activeView === 'credits'
                ...
                <div className="glass-panel rounded-3xl overflow-hidden relative">
                    ...
                    {creditSummary && (
                        <div className="grid grid-cols-3 gap-px bg-white/5 border-b border-white/5">
                            ...
                            <div className="bg-black/20 p-5">
                                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-400/60 mb-1">Total Balance</p>
                                <p className="text-2xl font-black text-emerald-400">${creditSummary.totalBalance?.toFixed(4)}</p>
                            </div>
                        </div>
                    )}
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="text-left border-b border-white/5 bg-white/[0.02]">
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Plan</th>
                                    <th
                                        onClick={() => { setCreditSort('total_used'); setCreditOrder(prev => creditSort === 'total_used' ? (prev === 'desc' ? 'asc' : 'desc') : 'desc'); }}
                                        className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-white/40 cursor-pointer hover:text-white transition-colors select-none"
                                    >
                                        <span className="flex items-center gap-1">
                                            Total Used
                                            <ArrowUpDown size={10} className={creditSort === 'total_used' ? 'text-red-400' : ''} />
                                        </span>
                                    </th>
                                    <th
                                        onClick={() => { setCreditSort('balance'); setCreditOrder(prev => creditSort === 'balance' ? (prev === 'desc' ? 'asc' : 'desc') : 'desc'); }}
                                        className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-white/40 cursor-pointer hover:text-white transition-colors select-none"
                                    >
                                        <span className="flex items-center gap-1">
                                            Balance
                                            <ArrowUpDown size={10} className={creditSort === 'balance' ? 'text-emerald-400' : ''} />
                                        </span>
                                    </th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Transactions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {creditUsage.map((u: any) => (
                                    <tr key={u.userId} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-bold text-white">{u.email}</p>
                                            <p className="text-[10px] text-white/30">{u.name}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider ${u.plan === 'Free'
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`text-sm font-black ${u.totalUsed > 0 ? 'text-red-400' : 'text-white/20'}`}>
                                                ${u.totalUsed.toFixed(4)}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-sm font-black text-emerald-400">
                                                ${u.currentBalance.toFixed(4)}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-xs text-white/40">{u.transactionCount}</span>
                                        </td>
                                    </tr>
                                ))}
